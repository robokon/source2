...
#define FLOOR_ONE_UP_OK_COUNT     200  /* どのくらい連続してジャイロが安定していたら登ったと判定するか <- これでぶつかってから真ん中までを調整 */
#define FLOOR_TWO_UP_OK_COUNT     300  /* どのくらい連続してジャイロが安定していたら登ったと判定するか <- これでぶつかってから真ん中までを調整 */
...
#define FLOOR_TWO_SPIN_VALUE  150   /* 450度回転 */

#define FLOOR_ZERO_WAITING_COUNT 1000
#define FLOOR_ONE_WAITING_COUNT  2000
#define FLOOR_TWO_WAITING_COUNT  3000
...
#else
/* log出力 */
int Gyro_Store_4ms[GYRO_AVE_STR_MAX] = {0};    /* 4msごとのジャイロセンサ値 */
int Gyro_Ave_log[GYRO_AVE_STR_MAX] = {0};      /* 平均値のlog格納先 */
int log_count_4ms = 0;
int Gyro_Ave_OK_COUNT_LOG[GYRO_AVE_STR_MAX] = {0}; /* フロア検知時のセンサー値がOKのカウント数 */
#endif

/* プロトタイプ宣言 */
void stair_Run(int forward_value, int turn_value);
int spin_func(int spin_end_value);
int stair_Stop(int Floor_Status);
...
void stair_main()
{
    ...
    if(Stair_Count == 0)
    {
        ...
        Stage_One_Spin_Status = SPIN_FINISH;
	...
	{
		/* 段位ごとの停止とフロアステータスの更新 */
		Floor_Status_tmp = stair_Stop(Floor_Status);
		if(Floor_Status_tmp != Floor_Status)
		{
			Floor_Status = Floor_Status_tmp;
			Waiting_Flag = WAITING_FLAG_OFF;
		}
	}
...
int stair_Stop(int Floor_Status)
{
    int32_t motor_ang_l, motor_ang_r;
    int gyro, volt;
	static int Floor_Status_stop_temp = 0;
    ...
    if(Floor_Status == STAGE_ZERO)
    {
        if(Waiting_Count_Flag_ZERO == WAITING_COUNT_FLAG_OFF)
        {
            Waiting_Count_Value = FLOOR_ZERO_WAITING_COUNT;
            Waiting_Count_Flag_ZERO = WAITING_COUNT_FLAG_ON;
        }

        Waiting_Count_Value--;
    	
    	if((Waiting_Count_Value % 100) == 0 )
    	{
    		ev3_speaker_set_volume(1);
    		ev3_speaker_play_tone(NOTE_DS4, 100);
    	}

        if(Waiting_Count_Value < 0)
        {
            Waiting_Flag = WAITING_FLAG_OFF;
            Floor_Status_stop_temp = STAGE_ONE;
        	ev3_speaker_set_volume(10);
    		ev3_speaker_play_tone(NOTE_DS4, 100);
        }
    }
    ...
    {
        if(Waiting_Count_Flag_ONE == WAITING_COUNT_FLAG_OFF)
        {
            Waiting_Count_Value = FLOOR_ONE_WAITING_COUNT;
            Waiting_Count_Flag_ONE = WAITING_COUNT_FLAG_ON;
        }

        Waiting_Count_Value--;

    	if((Waiting_Count_Value % 50) == 0 )
    	{
    		ev3_speaker_set_volume(1);
    		ev3_speaker_play_tone(NOTE_DS4, 100);
    	}
        ...
        {
            Waiting_Flag = WAITING_FLAG_OFF;
            Floor_Status_stop_temp = STAGE_TWO;
        	ev3_speaker_set_volume(10);
        	ev3_speaker_play_tone(NOTE_DS4, 100);
        }
    }
    else if(Floor_Status == STAGE_TWO)
    {
        ...
        {
            Waiting_Flag = WAITING_FLAG_OFF;
            Floor_Status_stop_temp = STAGE_LAST;
        	ev3_speaker_set_volume(10);
        	ev3_speaker_play_tone(NOTE_DS4, 100);
        }
    }
